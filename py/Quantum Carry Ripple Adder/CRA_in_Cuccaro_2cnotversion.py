...
def CRA_in_Cuccaro_2cnotversion(circ, q, c, n):
    '''
    This circuit is quantum carry-ripple adder using the binary quantum full adder proposed 
        in (Islam, et al. 2008). If user wants to add it in their circuits, they need 
    ...
    **Example:**
    ...
    circ = CRA_in_Cuccaro_2cnotversion(circ, q, c, n)

    '''
    for i in range(n):
        circ = BQFA_in_Cuccaro(circ, q[3*i],q[3*i+1],q[3*i+2],q[3*i+3]) 
        circ.measure(q[3*i+1],c[i])
    circ.measure(q[3*n],c[n])
    return circ
