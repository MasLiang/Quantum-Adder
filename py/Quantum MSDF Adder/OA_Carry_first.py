...
def OA_Carry_first(circ, q, c, n):
    '''
    This circuit is quantum online adder using Carry-first binary quantum full adder. If 
    ...
    **Example:**
    ...
    circ = OA_Carry_first(circ, q, c, n)

    '''
    for i in range(n):
        circ.x(q[5*i+1])
        circ = Carry_First_BQFA(circ, q[5*i], q[5*i+1], q[5*i+2], q_D=q[5*i+3])
    circ.measure(q[0],c[0])
    for i in range(n):
        circ.x(q[5*i+4])
        circ = Carry_First_BQFA(circ, q[5*i+3], q[5*i+4], q[5*i+5],c_Carry=2*i+2)
        circ.x(q[5*i+3])
