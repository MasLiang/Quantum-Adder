...
from BQFA_in_Islam import *

def QMA_in_Islam(circ, q, c, n):
    '''
    This circuit is quantum online adder using the binary quantum full adder proposed 
        in (Islam, et al. 2010). If user wants to add it in their circuits, they need 
    ...
    **Example:**
    ...
    circ = QA_in_Islam(circ, q, c, n)

    '''
    for i in range(n):
        circ.x(6*i+1)
        circ = BQFA_in_Islam(circ,q[6*i],q[6*i+1],q[6*i+2],q[6*i+3])
    circ.measure(q[2],c[0])
    for i in range(n-1):
        circ.x(6*i+4)
        circ = BQFA_in_Islam(circ,q[6*i+3],q[6*i+4],q[6*i+8],q[6*i+5])
        circ.x(q[6*i+8])
        circ.measure(q[6*i+8],c[2*i+1])
        circ.measure(q[6*i+5],c[2*i+2])
    circ.x(6*n-2)
    circ = BQFA_in_Islam(circ,q[6*n-3],q[6*n-2],q[6*n],q[6*n-1])
    circ.x(q[6*n])
