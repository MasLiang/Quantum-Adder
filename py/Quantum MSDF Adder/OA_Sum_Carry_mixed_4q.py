...
from BQFA_Sum_First_4q import *
from BQFA_in_Cuccaro import *

def OA_Sum_Carry_mixed_4q(circ, q, c, n):
    '''
    This circuit is quantum online adder using both the Sum-first binary quantum full 
        adder with 4 qubits and Carry-first binary quantum full adder with 4 qubits. 
    ...
    **Example:**
    ...
    circ = OA_Sum_Carry_mixed_4q(circ, q, c, n)
    ...
    for i in range(1,n):
        circ.x(q[5*i+1])
        circ = BQFA_Sum_first_left(circ, q[5*i], q[5*i+1], q[5*i+2])
    circ.x(q[3])
    circ = BQFA_in_Cuccaro(circ,q[5],q[3],q[2],q[4])
    circ = UMA_1(circ, q[0],q[1],q[2])
    circ.x(q[3])
    circ.measure(q[1],c[0])
    circ.measure(q[3],c[1])
    circ.measure(q[4],c[2])
    for i in range(1,n):
        circ = BQFA_Sum_first_right(circ, q[5*i],q[5*i+1],q[5*i+2])
        circ.x(q[5*i+3])
        circ = BQFA_in_Cuccaro(circ, q[5*i+5], q[5*i+3], q[5*i+2], q[5*i+4])
        circ.x(q[5*i+3])
        circ.measure(q[5*i+3],c[2*i+1])
        circ.measure(q[3*i+4],c[2*i+2])
