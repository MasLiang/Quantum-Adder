...
def OA_in_Cuccaro(circ, q, c, n):
    '''
    This circuit is quantum online adder using the binary quantum full adder proposed 
        in (Cuccaro, et al. 2008). If user wants to add it in their circuits, they need 
    ...
    **Example:**
    ...
    circ = OA_in_Cuccaro(circ, q, c, n)

    '''
    for i in range(n):
        circ.x(q[6*i+1])
        circ = BQFA_in_Cuccaro(circ, q[6*i], q[6*i+1], q[6*i+2], q[6*i+3])
    for i in range(n-1):
        circ.x(q[6*i+4])
        circ = BQFA_in_Cuccaro(circ, q[6*i+3], q[6*i+4], q[6*i+7], q[6*i+5])
        circ.x(q[6*i+4])
        circ.measure(q[6*i+4],c[2*i+1])
        circ.measure(q[6*i+5],c[2*i+2])
    circ.x(q[6*n-2])
    circ = BQFA_in_Cuccaro(circ, q[6*n-3], q[6*n-2], q[6*n], q[6*n-1])
    circ.x(q[6*n-2])
