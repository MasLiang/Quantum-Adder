...
def OA_Sum_First_4q(circ, q, c, n):
    '''
    This circuit is quantum online adder using the Sum-first binary quantum full 
        adder with 4 qubits. If user wants to add it in their circuits, they need 
    ...
    **Example:**
    ...
    circ = OA_Sum_First_4q(circ, q, c, n)

    '''
    for i in range(n):
        circ.x(q[6*i+1])
        circ = BQFA_Sum_first_4q(circ, q[6*i], q[6*i+1], q[6*i+2], q[6*i+4])
    circ.barrier()
    for i in range(n-1):
        circ.x(q[6*i+3])
        circ = BQFA_Sum_first_4q(circ, q[6*i+2], q[6*i+3], q[6*i+10], q[6*i+5])
        circ.x(q[6*i+5])
        circ.measure(q[6*i+10],c[2*i+1])
        circ.measure(q[6*i+5],c[2*i+2])
    circ.x(q[6*n-3])
    circ = BQFA_Sum_first_4q(circ, q[6*n-4], q[6*n-3], q[6*n], q[6*n-1])
    circ.x(q[6*n-1])
